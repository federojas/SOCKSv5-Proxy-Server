...
static char *dest_to_human(enum socks5_addr_type dest_addr_atyp,
...
static char *dest_to_human(enum socks5_addr_type dest_addr_atyp,
    ...
    case SOCKS5_REQ_ADDRTYPE_IPV4:
        ...
        inet_ntop(AF_INET, &(((struct sockaddr_in *)&dest_addr.ipv4)->sin_addr),
                  to_return, INET_ADDRSTRLEN);
        break;
    case SOCKS5_REQ_ADDRTYPE_IPV6:
        to_return = (char *)malloc(INET6_ADDRSTRLEN * sizeof(char));
        inet_ntop(AF_INET6,
                  &(((struct sockaddr_in6 *)&dest_addr.ipv6)->sin6_addr),
                  to_return, INET6_ADDRSTRLEN);
        break;
...
static void print_log_data(log_data *log_data, log_type type) {
    ...
    size_t string_addr_length = log_data->client_addr.ss_family == AF_INET
                                    ? INET_ADDRSTRLEN
                                    : INET6_ADDRSTRLEN;
    char client_string_addr[string_addr_length];
    sockaddr_to_human(client_string_addr, string_addr_length,
                      &log_data->client_addr);

    char *dest_string_addr =
        dest_to_human(log_data->dest_addr_atyp, log_data->dest_addr);
    ...
    case AUTH_LOG_DATA:
        if (log_data->username != NULL) {
            ...
            log_print(INFO, NO_AUTH_FORMAT, log_data->date, client_string_addr,
                      dest_string_addr, dest_port, log_data->response_status);
        }
