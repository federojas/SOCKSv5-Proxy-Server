...
dog_client_command dog_client_commands[] = {
    {.name = "list", .usage = "list", .builder = get_list_builder, .nparams = 0, .description="Returns a list of all users registered on the server", .on_success_message="TODO" },
    {.name = "hist", .usage = "hist", .builder = get_historic_conn_builder, .nparams = 0, .description="Returns the amount of historic connections over the server", .on_success_message="TODO" },
    {.name = "conc", .usage = "conc", .builder = get_conc_conn_builder, .nparams = 0, .description="Returns the amount of concurrent connections over the server", .on_success_message="TODO" },
    {.name = "bytes", .usage = "bytes", .builder = get_bytes_transf_builder, .nparams = 0, .description="Returns the amount of bytes transfered over the server", .on_success_message="TODO" },
    {.name = "checksniff", .usage = "checksniff", .builder = get_sniffing_builder, .nparams = 0, .description="Returns the status of the password disector over the server", .on_success_message="TODO" },
    {.name = "checkauth", .usage = "checkauth", .builder = get_auth_builder, .nparams = 0, .description="Returns the status of authentication over the server", .on_success_message="TODO" },
    {.name = "getpage", .usage = "getpage", .builder = get_user_page_size_builder, .nparams = 0, .description="Returns the amount of users per page (max 200)", .on_success_message="TODO" },
    {.name = "add", .usage = "add user:pass", .builder = alter_add_user_builder, .nparams = 1, .description="Command to add a user", .on_success_message="TODO" },
    {.name = "del", .usage = "del user:pass", .builder = alter_del_user_builder, .nparams = 1, .description="Command to delete a user", .on_success_message="TODO" },
    {.name = "sniff", .usage = "sniff on / sniff off", .builder = alter_toggle_sniff_builder, .nparams = 1, .description="Command to toggle password disector over the server", .on_success_message="TODO" }, 
    {.name = "auth", .usage = "auth on / auth off", .builder = alter_toggle_auth_builder, .nparams = 1, .description="Command to toggle authentication over the server", .on_success_message="TODO" },
    {.name = "setpage", .usage = "setpage n (n between 1 and 200)", .builder = alter_user_page_size_builder, .nparams = 1, .description="Command to set page size", .on_success_message="TODO" },
};
...
int main(int argc, const char *argv[])
{
    ...
    for(;!done;) {
        ...
        else
        {
            ...
            resp_size = recvfrom(sockfd, (char *)buffer_in, BUFFER_SIZE,
                                 MSG_WAITALL, (struct sockaddr *)&serv_addr6,
                                 &len);
        }
        ...
        response_handler(dog_req, dog_res, dog_client_commands[i].on_success_message);
    }
