...
static request_state dstaddr(request_parser *p, const uint8_t c) {
    ...
    case SOCKS5_REQ_ADDRTYPE_IPV4:
        p->request->dest_addr.ipv4.sin_addr.s_addr =
            (p->request->dest_addr.ipv4.sin_addr.s_addr << 8) + c;
        p->readBytes++;
        // finalice la lectura y pasamos al puerto o seguimos
        if (remaining_is_done(p)) {
            p->request->dest_addr.ipv4.sin_addr.s_addr =
                htonl(p->request->dest_addr.ipv4.sin_addr.s_addr);
        }
        break;
    case SOCKS5_REQ_ADDRTYPE_IPV6:
        ((uint8_t *)&(p->request->dest_addr.ipv6.sin6_addr))[p->readBytes++] =
            c;
        // no necesito una func como htonl porque struct in6_addr se representa
        // como uint8_t u6_addr8[16];
        break;
    case SOCKS5_REQ_ADDRTYPE_DOMAIN:
...
bool request_parser_consume(buffer *b, request_parser *p, bool *errored) {
    ...
    uint8_t byte;
    while (!request_parser_is_done(p->current_state, errored) &&
           buffer_can_read(b)) {
        byte = buffer_read(b);
...
int request_marshall(buffer *b, const enum socks5_response_status status,
                     const enum socks5_addr_type atyp,
                     const union socks5_addr addr, const in_port_t port) {
    size_t n;
    ...
    /* vamos switcheando entre los diferentes tipos, y en base al tipo de
    address sabemos que hay que ponerle a los lugares restantes de buff*/
    switch (atyp) {
    case SOCKS5_REQ_ADDRTYPE_IPV4:
        addr_size = 4;
        len += addr_size;
...
enum socks5_response_status cmd_resolve(struct socks5_request *request,
    ...
    *domain = AF_INET;
    ...
    case SOCKS5_REQ_ADDRTYPE_DOMAIN: {
        ...
        memcpy((char *)&request->dest_addr.ipv4.sin_addr, *hp->h_addr_list,
               hp->h_length);
    }
